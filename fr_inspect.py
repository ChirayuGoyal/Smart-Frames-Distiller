"""
fr_inspect.py — Visually review untagged face UUIDs (Stack B).
...
from faces.store import MilvusFaceStore, require_site_id, resolve_site_camera
...
def make_crops(video: str, cfg: dict, out_dir: Path, per_uuid: int = 1) -> None:
    ...
    try:
        ...
        rows = store.query_paged(store.untagged_expr(site_id), ["id", "embedding"])
        if not rows:
        ...
        if missing:
            log.info("%d UUIDs had no match in video:", len(missing))
            for u in missing[:20]:
                print(f"  {u}")
    finally:
        store.close()
...
def make_sheet(in_dir: Path, out_path: Path, cols: int = 10, cell_size: int = 150, label_h: int = 24) -> None:
    ...
    log.info("Contact sheet saved: %s", out_path)
...
def main() -> None:
    ...
    p_both = sub.add_parser("both", help="Generate crops then contact sheet")
    p_both.add_argument("-c", "--config", type=Path, default=_BASE / "config.json")
    p_both.add_argument("video", help="Input video path")
    p_both.add_argument("--per-uuid", type=int, default=1)
    p_both.add_argument("--out-dir", type=Path, default=Path("uuid_crops"))
    ...
    p_both.add_argument("--site-id", default=None)

    p_crops = sub.add_parser("crops", help="Save face crops for untagged UUIDs")
    p_crops.add_argument("-c", "--config", type=Path, default=_BASE / "config.json")
    ...
    p_crops.add_argument("--site-id", default=None)

    p_sheet = sub.add_parser("sheet", help="Build contact sheet from existing crops")
    p_sheet.add_argument("-c", "--config", type=Path, default=_BASE / "config.json")
    ...
    if args.cmd in ("both", "sheet"):
        ...
        make_sheet(in_dir, out_p, cols=args.cols, cell_size=args.cell_size)
